export default {
  ...
  'repository.nnecec-react.desc': '由 Parcel 构建，React 框架编写的个人网站，使用了 TypeScript。<br/><br/>选择 Parcel 主要为了尝试与 Webpack 不同的构建工具，对于小型网站零配置预估也是可行的，可是零配置却让有些需要配置的地方感到不方便使用。<br/>项目主体采用 TypeScript + React 开发，MobX 进行状态管理。样式使用 PostCSS 作为预处理工具，但在开发中，越来越觉得需要样式组件化，一方面方便开发，一方面局部样式不需要考虑对全局的影响，于是逐渐使用 styled-components 进行替换。多语言的功能使开发变得繁琐，但是结果表现确实非常出色实用。',
